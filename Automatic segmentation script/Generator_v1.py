...
# 数据生成器类，继承自 keras.utils.Sequence，用于生成批量数据
class Patient_data_generator(keras.utils.Sequence):
    ...
    def __init__(self,patient_dict, batch_size=8, image_size=512,tumor_size_low_lim = 0,sl_th = None, zero_sl_ratio = 1, model_params={},
        ...
        # 初始化参数
        self.patient_dict = patient_dict  # 病人数据字典，键为文件名，值为图像和掩码路径
        self.filenames = list(patient_dict.keys())  # 文件名列表
        self.batch_size = batch_size  # 批量大小
        self.image_size = image_size  # 图像大小
        self.shuffle = shuffle  # 是否打乱数据
        self.augment = augment  # 是否进行数据增强
        self.hu_intensity = hu_intensity  # 是否调整HU值
        self.predict = predict  # 是否为预测模式
        self.normalize = normalize  # 是否进行归一化
        self.norm_coeff =norm_coeff  # 归一化系数
        self.equalize_hist=equalize_hist  # 是否进行直方图均衡化
        self.calculate_snr = calculate_snr  # 是否计算信噪比
        self.use_window = use_window  # 是否使用窗位窗宽
        self.window_params = window_params  # 窗位窗宽参数
        self._tum_size = tumor_size_low_lim  # 肿瘤大小下限
        self._slice_thickness = sl_th  # 切片厚度
        self.zero_sl_ratio = zero_sl_ratio  # 无肿瘤切片比例
        self.resample_int_val = resample_int_val  # 是否重采样强度值
        self.step = resampling_step  # 重采样步长
        self.img_only = img_only  # 是否只加载图像
        self.tum_size_distr=[]  # 肿瘤大小分布
        self.tum_volume_distr={}  # 肿瘤体积分布
        self.zero_dict={}  # 无肿瘤切片字典
        self.tum_dict={}  # 有肿瘤切片字典
        self.reshape = reshape  # 是否调整图像大小
        self.verbosity=verbosity  # 是否输出详细信息
        self.extract_lungs = extract_lungs  # 是否提取肺部
        self.model_params=model_params  # 模型参数
        self.on_epoch_end()  # 初始化epoch结束操作
        self.__epoch_zero_dict = None  # 当前epoch的无肿瘤切片字典
        self.__epoch_tum_dict = None  # 当前epoch的有肿瘤切片字典
        self._normalization_init()  # 初始化归一化
        self._patients_init(size_eval)  # 初始化病人数据
    ...
    def _load_patient(self, filename):
        ...
        img = sitk.GetArrayFromImage(sitk.ReadImage(self.patient_dict[filename][0]))  # 读取图像
        msk = sitk.GetArrayFromImage(sitk.ReadImage(self.patient_dict[filename][1]))  # 读取掩码
        if self.verbosity:
            print('-'*7,'Img spacing ',sitk.ReadImage(self.patient_dict[filename][0]).GetSpacing(),'-'*7)
            print('Image shape:',img.shape)

        # 调整HU值
        if self.hu_intensity:# 设置下限为-1000 HU
            img[img <-1000] = -1000
            if self.verbosity:
    ...
    def __len__(self):
        if self.predict:
            ...
            # 根据有肿瘤切片和无肿瘤切片的数量以及批量大小计算生成器的长度
